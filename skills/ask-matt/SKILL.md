...
- **Something's broken** → **`/diagnosing-bugs`**. For the hard ones: the bug that resists a first glance, the intermittent flake, the regression that crept in between two known-good states. It refuses to theorise until it has a **tight feedback loop** — one command that already goes red on *this* bug — then fixes with a regression test.Detour through **`/bug-reproduction-brief`** first: turn a vague report into a minimal evidence-backed reproduction before fixing. Its post-mortem hands off to **`/improve-codebase-architecture`** when the real finding is that there's no good seam to lock the bug down.
...
- **`web-design-guidelines`** — check UI against web accessibility (ARIA, focus), forms, touch interactions, typography, and theme standards.
- **`writing-guidelines`** — audit docs and prose for Vercel writing handbook compliance (tone, voice, clarity).
- **`typescript-best-practices`** — strict type safety, async execution, path mapping, and ESM conventions.
- **`python-best-practices`** — PEP 8 style, strict typing/mypy, async asyncio, and pytest test layouts.
- **`vercel-react-best-practices`** — React and Next.js performance optimization guidelines.
- **`vercel-react-native-skills`** — React Native and Expo performance and animation practices.
- **`vercel-react-view-transitions`** — smooth page transitions and shared element motions using native APIs.
...
- **`/code-tour`** — generate `.tour` walkthrough files for onboarding or describing architectural regions.
- **`/convert-pdf-to-md` / `/convert-excel-to-md` / `/convert-word-to-md`** — convert binary assets (.pdf, .xlsx, .docx) into clean markdown for context-ingestion.
- **`/git-commit`** — conventional commit analysis, staging, and message generation.
- **`/deploy-to-vercel`** — build and deploy applications or websites directly to Vercel.
- **`/vercel-cli-with-tokens`** — non-interactive Vercel deployments and environment config using access tokens.
